...
<div class="allInline">
  ...
  {F}_\text
  {high}$ and $\dot{F}_\text{low}$ take into account a $v^3$ normalization
  factor so that the baselines remain valid independent of airspeed, which may
  change throughout the simulation. To recompute force from the baseline space,
  a factor of $v^3$ is applied. Once calibrated, force savings are quickly
  estimated by interpolating the normalized observed horizontal resistance force
  between the two bounds and scaling the force savings regression model
  accordingly.
</div>
