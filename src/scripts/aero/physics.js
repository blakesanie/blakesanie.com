...
function stream() {
  ...
  for (let x = 1; x < xdim; x++) {
    for (let y = 1; y < ydim; y++) {
      ...
      nNW[fromTopLeft - xdim - 1] = nNW[fromTopLeft];
      // towards north west
      nS[fromBottomRight + xdim] = nS[fromBottomRight];
      nE[fromBottomRight + 1] = nE[fromBottomRight];
      nSE[fromBottomRight + xdim + 1] = nSE[fromBottomRight];
      // towards north east
      nSW[fromBottomLeft + xdim - 1] = nSW[fromBottomLeft];
      // towards south west
...
function colorSquare(x, y, r, g, b, a) {
  for (var py = y * pxPerSquare; py < (y + 1) * pxPerSquare; py++) {
