...
const runMinifierPipeline = async () => {
  ...
  const totalPercentSavings =
    totalBytesBefore > 0 ? ((totalSavings / totalBytesBefore) * 100).toFixed(1) : 0;
  ...
  console.log(
    `Total space saved        : \x1b[32m${formatBytes(totalSavings)} (-${totalPercentSavings}%)\x1b[0m\n`,
  );
