...
function radio() {
  ...
  var five = [
    87, 104, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 85, 118,
    102, 103,
  ];
  //unknown = 117
  $(".item").each(function (i) {
...
function change() {
  ...
  topLeft();
  $("#percent").html(
    Math.floor(Math.max(100, ($("input").val() / Twidth) * 100)) + "%",
  );
  font();
}
    ...
    success: function (data) {
      $(".item").each(function (i) {
        ...
        if (i != 56 && i != 74) {
          ...
          if (
            data.elements[num - 1].atomic_mass.toString().length <=
            data.elements[num - 1].atomic_mass.toFixed(4).toString().length
          ) {
            if (
              data.elements[num - 1].atomic_mass.toString().indexOf(".") ==
                -1 ||
              num > 83
            ) {
              $(this).append(
                "<h2>" +
                  "(" +
                  data.elements[num - 1].atomic_mass +
                  ")" +
                  "</h2>",
              );
            } else {
              $(this).append(
                "<h2>" + data.elements[num - 1].atomic_mass + "</h2>",
              );
            }
          } else {
            if (
              ...
              $(this).append(
                "<h2>" +
                  "(" +
                  data.elements[num - 1].atomic_mass.toFixed(4) +
                  ")" +
                  "</h2>",
              );
            } else {
...
$(".item:not(.blank)").click(function () {
  ...
  $("iframe").attr(
    "src",
    "https://en.wikipedia.org/wiki/" + $(this).find("h3").text() + "",
  );
});
...
$("#icon").click(function () {
  if (exit === false) {
    ...
    $("#line:nth-of-type(1)").css("transform", "rotate(45deg)");
    $("#line:nth-of-type(3)").css("transform", "rotate(-45deg)");
    $("#modeHolder").css({
      width: "220px",
      height: Math.min(300, $(window).height() - 274) + "px",
      opacity: "1",
    ...
    $("#line:nth-of-type(1)").css("transform", "rotate(0)");
    $("#line:nth-of-type(3)").css("transform", "rotate(0)");
    $("#modeHolder").css({
      width: "120px",
      height: "0",
      opacity: "0",
